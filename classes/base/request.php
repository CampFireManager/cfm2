...
class base_request
{
    ...
    function getRequest()
    {
        ...
        $match = preg_match('/^([^\?]+)/', $handler->arrRequestData['full'], $matches);
        if ($match > 0) {
        ...
        if (isset($_SERVER['REQUEST_METHOD']) && isset($_SERVER['SCRIPT_NAME'])) {
            ...
            $char = 0;
            while (isset($path_elements[$char]) && $path_elements[$char] == $script_elements[$char]) {
                $char++;
            }
        ...
        if (count($arrLastUrlItem) > 1) {
            ...
            $handler->arrRequestData['path_items'][count($handler->arrRequestData['path_items'])-1] = '';
            ...
            foreach ($arrLastUrlItem as $key=>$UrlItem) {
                ...
                if ($key + 1 == count($arrLastUrlItem)) {
                    ...
                    if ($handler->arrRequestData['path_items'][count($handler->arrRequestData['path_items'])-1] != '') {
                        $handler->arrRequestData['path_items'][count($handler->arrRequestData['path_items'])-1] .= '.';
                    }
                    $handler->arrRequestData['path_items'][count($handler->arrRequestData['path_items'])-1] .= $UrlItem;
                }
            }
        }
        ...
        if ($format != '') {
            ...
            switch (strtolower($format)) {
                ...
                default:
                    $handler->arrRequestData['arrAcceptTypes']['unknown/' . $format] = 1;
            }
        }
        ...
        // Finally, let's check if they gave us HTTP credentials
